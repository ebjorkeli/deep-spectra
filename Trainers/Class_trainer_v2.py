...
class Trainer:
    def __init__(self, model, train_data, val_data, config):
        ...
        self.num_targets = len(config['labels']) + 1 # n labels + 1 spectra
...
### Loss functions
    ...
    def random_labels(self,data):
        "Returns random labels for machine and patient (data[1] and data[2]) and normal labels for glioma data[0]."
        data = np.array(data)
        new_data = np.random.random(data.shape)
        for i in range(1,data.shape[1]):
            new_data[:,i] *= np.max(data[:,i])
...
### Step functions
    def train_stp(self, epoch, optimizer):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            with tf.GradientTape(persistent=True) as t:
                ...
                if self.model_type == 'autoencoder':
                    losses = self.training_loss(prediction, [self.random_labels(data[2]), data[0]])  ## labels, time_spectra
        ...
        #stddev = 1 / ((1 + epoch) ** 0.75)
        #gradients = [tf.add(gradient, tf.random.normal(stddev=stddev, mean=0., shape=gradient.shape)) for gradient in
        #             gradients]
   ...
   # @tf.function
    def train(self):
        ...
        for epoch in range(self.eps):
            ...
            ###    UPADATE STUFF   ###
            for step in range(self.num_spec//self.bs + 1):
        ...
        if self.save:
            try:
                self.model.save('{}'.format(self.name + '_' + self.time))
            except ValueError:
