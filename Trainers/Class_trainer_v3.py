...
### Step functions
    def train_stp(self, epoch, optimizer):
        ...
        # Pretraining + alternating training
        if epoch % (self.ur+1) != 0 or epoch < self.config['pretrain']:
            for layer in self.model.layers[3:14]:
                layer.trainable = False
            opt = optimizer[0]
        elif epoch % (self.ur+1) != 0 and epoch > self.config['pretrain']:
            for layer in self.model.layers[3:14]:
               layer.trainable = True
            self.model.layers[2].trainable = False
            self.model.layers[5].trainable = False
            opt = optimizer[1]
        else:
            for layer in self.model.layers[3:14]:
                layer.trainable = False
